...
.command{
  ...
  executeCommand(text) {
    ...
    if( typeof CommandList[command].action != "undefined" ){//If command has to manipulate component
        let result = CommandList[command].action(subCommand) //Let command manipulate state
        
        if( "updateState" in result ){ //If command needs to update the state
            Object.keys(result.updateState).forEach((key)=>this[key] = result.updateState[key]);
        }

        if( "output" in result ){ //If command has output, show it
